...
# Historical cached-label route used by the benchmark snapshots.
project_root = str(Path(__file__).resolve().parents[2])
sys.path.append(project_root)

from mycaafe.run_llm_code import run_llm_code
from mycaafe import CAAFEClassifier as Stage1Classifier
...
_compare_cluster_utils_path = (
    Path(__file__).resolve().parents[3]
    / "autologic"
    / "ensemble"
    / "cluster_ensemble_utils.py"
)
_spec = importlib.util.spec_from_file_location("compare_cluster_ensemble_utils", str(_compare_cluster_utils_path))
_compare_cluster_utils = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_compare_cluster_utils)
select_top_models_with_adaptive_threshold = _compare_cluster_utils.select_top_models_with_adaptive_threshold
cluster_ensemble_caps = _compare_cluster_utils.cluster_ensemble_caps
get_param_prompt = _compare_cluster_utils.get_param_prompt
distill_to_student_clustering = _compare_cluster_utils.distill_to_student_clustering
stacking_clustering_with_calibration = _compare_cluster_utils.stacking_clustering_with_calibration
consensus_ensemble_topk = _compare_cluster_utils.consensus_ensemble_topk
