...
def _panelb_ece_bins(y_true, prob, n_bins=10):
    y_true = np.asarray(y_true).astype(int).reshape(-1)
    prob = np.clip(np.asarray(prob, dtype=float).reshape(-1), 1e-6, 1.0 - 1e-6)
    ...
    for b in range(n_bins):
        ...
        out.append({
            ...
            "empirical_accuracy": float(y_true[mask].mean()) if mask.any() else float("nan"),
        })
    return out


def _panelb_binary_y(y):
    ...
    return LabelEncoder().fit_transform(values.astype(str)).astype(int)
    ...
    for exp in range(args.exam_iterations):
        ...
        for config in configurations:
            ...
            for meta_model in metaModelName_list:
                ...
                panelb_nll = float("nan")
                panelb_bins = []
                try:
                    panelb_y_test = _panelb_binary_y(test_aug_y)
                    panelb_prob = result["meta_model"].predict_proba(result["test_meta"])[:, 1]
                    panelb_prob = np.clip(np.asarray(panelb_prob, dtype=float), 1e-6, 1.0 - 1e-6)
                    panelb_nll = float(log_loss(panelb_y_test, panelb_prob, labels=[0, 1]))
                    panelb_bins = _panelb_ece_bins(panelb_y_test, panelb_prob, n_bins=10)
                except Exception:
                    pass
                logger.log(
                    stage="ensemble_eval",
                    ...
                    accuracy=stacking_metrics.get("accuracy", None),
                    f1=stacking_metrics.get("f1", None),
                    precision=stacking_metrics.get("precision", None),
                    recall=stacking_metrics.get("recall", None),
                    nll=panelb_nll,
                    reliability_bins=panelb_bins,
                    calibration_check_triggered=1 if config["calibrate"] else 0,
                )
