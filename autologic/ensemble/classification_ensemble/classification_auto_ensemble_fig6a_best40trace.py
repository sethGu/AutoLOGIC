...
def export_fig6a_source_from_jsonl(jsonl_path, output_dir, display_dataset=None):
    ...
    trace_csv = os.path.join(output_dir, "fig6a_trace.csv")
    trace_fields = [
        "dataset", "dataset_internal", "run_id", "round", "val_metric_main", "val_score",
        "val_score_before_opt", "best_score_so_far", "val_trust_metric",
        "val_trust", "distill_val_auc", "llm_tokens", "code_len",
        "base_model_count", "feature_count", "status",
    ]
    with open(trace_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=trace_fields)
        writer.writeheader()
        for rec in records:
            if rec.get("stage") == "model_iter":
                row = {k: rec.get(k) for k in trace_fields}
                row["dataset_internal"] = rec.get("dataset")
                row["dataset"] = display_dataset or rec.get("dataset")
                writer.writerow(row)

    final_csv = os.path.join(output_dir, "fig6a_final_box.csv")
    final_fields = [
        "dataset", "dataset_internal", "run_id", "round", "config_key", "config_name",
        "distill_used", "calibrate_proba", "meta_model",
        "test_metric_main", "test_score", "test_auc", "test_acc", "test_f1",
        "test_precision", "test_recall", "test_trust_metric", "test_trust",
        "test_ece", "brier", "test_brier", "calibration_check_triggered",
        "base_model_count", "feature_count", "status", "error",
    ]
    final_rows = []
    with open(final_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for rec in records:
            if rec.get("stage") != "ensemble_eval":
                continue
            meta_config = rec.get("meta_config", {}) or {}
            row = {k: rec.get(k) for k in final_fields}
            row["dataset_internal"] = rec.get("dataset")
            row["dataset"] = display_dataset or rec.get("dataset")
            row["config_key"] = meta_config.get("config_key")
            row["config_name"] = meta_config.get("config_display_name") or meta_config.get("config_name")
            row["distill_used"] = meta_config.get("distill_used")
            row["meta_model"] = meta_config.get("meta_model")
            row["calibrate_proba"] = meta_config.get("calibrate_proba")
            row["test_auc"] = rec.get("test_score")
            row["test_acc"] = rec.get("test_acc")
            row["test_f1"] = rec.get("test_f1")
            row["test_precision"] = rec.get("test_precision")
            row["test_recall"] = rec.get("test_recall")
            row["test_ece"] = rec.get("test_trust")
            row["test_brier"] = rec.get("brier")
            writer.writerow(row)
            final_rows.append(row)

    best_csv = os.path.join(output_dir, "fig6a_best_for_panel.csv")
    best_fields = final_fields + ["selection_rule"]
    valid_rows = []
    for row in final_rows:
        try:
            auc = float(row.get("test_auc"))
        except Exception:
            continue
        if np.isfinite(auc):
            valid_rows.append((auc, row))
    with open(best_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=best_fields)
        writer.writeheader()
        if valid_rows:
            best = max(valid_rows, key=lambda x: x[0])[1]
            out = dict(best)
            out["selection_rule"] = "max test_auc among four AutoLOGIC compare final configs, matching best40 classification selection"
            writer.writerow(out)

    return trace_csv, final_csv, best_csv
...
# 学生网络定义
# =========================
class _Fig6aPandasIOModelWrapper:
    """Restore pandas inputs during stacking OOF refits for LLM wrappers."""
    ...
    def _as_frame(self, X):
        if isinstance(X, pd.DataFrame):
            return X.copy()
        arr = np.asarray(X)
        cols = self.feature_columns[: arr.shape[1]] if arr.ndim == 2 else self.feature_columns
    ...
    def fit(self, X, y):
        self.model.fit(self._as_frame(X), self._as_series(y))
        return self
    ...
    def predict_proba(self, X):
        proba = np.asarray(self.model.predict_proba(self._as_frame(X)))
        if proba.ndim == 1:
            proba = np.column_stack([1.0 - proba, proba])
        return proba


def _wrap_models_for_fig6a_stacking(models, feature_columns):
    return [_Fig6aPandasIOModelWrapper(m, feature_columns) for m in models]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fig6a_output_dir', default="", type=str, help='Output directory for Fig. 6a source records')
    parser.add_argument('--fig6a_run_id', default="", type=str, help='Stable run id for Fig. 6a source records')
    parser.add_argument('--fig6a_display_dataset', default="", type=str, help='Dataset label written to Fig. 6a source CSVs')
    ...
    fig6a_display_dataset = args.fig6a_display_dataset.strip() or ds_name
    # cd1 cc1 ld1  cc2 cd2 cf1 balance-scale ds_credit
    print(f"=========== Dataset {ds_name} ===========")
    fig6a_run_id = args.fig6a_run_id or datetime.now().strftime("%Y%m%d_%H%M%S")
    if args.fig6a_output_dir:
        fig6a_output_dir = os.path.abspath(args.fig6a_output_dir)
    else:
        fig6a_output_dir = os.path.join(
            os.path.abspath(os.path.join(project_root, "..")),
            f"results_fig6_panel_a_bestperf_classification_{fig6a_run_id}",
        )
    os.makedirs(fig6a_output_dir, exist_ok=True)
    logger = QuantLogger(
        task="classification",
        dataset=ds_name,
        out_dir=fig6a_output_dir,
        run_id=fig6a_run_id,
    )
    try:
        import hashlib
        source_code_hash = hashlib.sha256(open(__file__, "rb").read()).hexdigest()
    except Exception:
        source_code_hash = None
    manifest_path = os.path.join(fig6a_output_dir, "run_manifest.json")
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(
            {
                "created_at": _fig6a_now_iso(),
                "dataset": ds_name,
                "dataset_display": fig6a_display_dataset,
                "run_id": fig6a_run_id,
                "source_code_path": os.path.abspath(__file__),
                "source_code_sha256": source_code_hash,
                "base_code_source": "results_full_performance_cached_20260526_v6/script_copies/compare_classification/classification_auto_ensemble.py",
                "stage1_guard": "Uses current autoLOGIC_compare/mystage1/stage1_evaluate.py numeric coercion guard for generated feature selection.",
                "best40_creditg_route_note": "The best40 workbook row named credit-g was run through internal dataset argument ds_credit in run_full_jobs.ps1.",
                "project_root": project_root,
                "jsonl_path": logger.path,
                "fig6a_output_dir": fig6a_output_dir,
                "llm": args.llm,
                "default_seed": args.default_seed,
                "exam_iterations": args.exam_iterations,
                ...
                "api_base_url_from_env": bool(env_url),
            ...
            indent=2,
            ensure_ascii=False,
            default=_fig6a_jsonable,
        )
    print(f"Fig6a source output directory: {fig6a_output_dir}")
    ...
    for exp in range(args.exam_iterations):
        ...
        config_metadata = [
            {
                "key": "teacher_no_distill_no_calibration",
                "display_name": "teacher ensemble + no calibration",
                "distill_used": False,
            },
            {
                "key": "teacher_no_distill_sigmoid_calibration",
                "display_name": "teacher ensemble + sigmoid calibration",
                "distill_used": False,
            },
            {
                "key": "student_distill_sigmoid_calibration",
                "display_name": "student distillation + sigmoid calibration",
                "distill_used": True,
            },
            {
                "key": "student_distill_no_calibration",
                "display_name": "student distillation + no calibration",
                "distill_used": True,
            },
        ]
        for cfg, meta in zip(configurations, config_metadata):
            cfg.update(meta)

        for config in configurations:
            print(f"\n【第 {exp + 1} 轮】正在执行 {config['name']} ...")
            ensemble_models = _wrap_models_for_fig6a_stacking(
                config['models'],
                train_aug_x.columns.tolist(),
            )
            ...
            for meta_model in metaModelName_list:
                try:
                    ...
                    logger.log(
                        ...
                        task_type="classification",
                        feature_count=int(train_aug_x.shape[1]),
                        base_model_count=int(len(ensemble_models)),
                        meta_config={
                            "config_key": config.get("key"),
                            "config_display_name": config.get("display_name"),
                            "config_name": config["name"],
                            "distill_used": bool(config.get("distill_used", False)),
                ...
                logger.log(
                    ...
                    test_acc=stacking_metrics.get("accuracy", None),
                    test_f1=stacking_metrics.get("f1", None),
                    test_precision=stacking_metrics.get("precision", None),
                    test_recall=stacking_metrics.get("recall", None),
...
{[f"第 {i+1} 轮: {t:.2f} 秒" for i, t in enumerate(feat_time_list)]}
    ...
    # 保存结果到txt文件
    result_dir = os.path.join(fig6a_output_dir, "text_result")
    ...
    try:
        ...
        print(f"\n保存结果文件失败: {e}")
    logger.close()
    trace_csv, final_csv, best_csv = export_fig6a_source_from_jsonl(
        logger.path,
        fig6a_output_dir,
        display_dataset=fig6a_display_dataset,
    )
    print(f"Fig6a JSONL saved to: {logger.path}")
    print(f"Fig6a trace source saved to: {trace_csv}")
    print(f"Fig6a final box source saved to: {final_csv}")
    print(f"Fig6a selected best source saved to: {best_csv}")
