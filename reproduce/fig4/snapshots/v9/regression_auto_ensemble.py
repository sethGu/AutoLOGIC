...
def generate_feat(
    ...
    final_columns = [
        c for c in stage1_clf.final_columns
        if c in df_train_aug.columns and c in df_test_aug.columns
    ]
    if target_column_name in df_train_aug.columns and target_column_name not in final_columns:
        final_columns.append(target_column_name)
    missing_columns = [c for c in stage1_clf.final_columns if c not in final_columns]
    if missing_columns:
        print(f"Skipping invalid generated feature columns: {missing_columns}")
    if target_column_name not in final_columns:
        final_columns.append(target_column_name)

    df_train_aug = df_train_aug[final_columns]
    df_test_aug = df_test_aug[final_columns]
